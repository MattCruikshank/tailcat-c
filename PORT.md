# Porting tailcat to C

**This is the engineering record of the port: what was built, how it was
verified, and every bug the verification caught. For what the program is and
how to use it, see [README.md](README.md).**

It is kept because the interesting part of this project is not the feature
list — it is the evidence. A from-scratch implementation of somebody else's
cryptographic protocols is worth exactly as much as its proof that it agrees
with them, and that proof is long, so it lives here rather than in a manual.

`tailcat-c` is a rewrite of [tailcat](https://github.com/tailscale/tailcat) in
C11, built with the [Cosmopolitan](https://github.com/jart/cosmopolitan) C
compiler into a single **fat Actually Portable Executable** — one binary that
runs on Linux, macOS, Windows, FreeBSD, OpenBSD and NetBSD, on both x86_64 and
aarch64.
...
| `readme` | ✅ (embeds doc/usage.md) | ✅ (embeds README.md) |
...
Tailscale-authored code, and [README.md](README.md) follows the structure of
upstream's README, so [LICENSE](LICENSE) is upstream's own file copied
unchanged -- the copyright notice, the conditions and the disclaimer that a
source redistribution has to carry.

It was missing until late in the project. `upstream-tailcat/` is in
`.gitignore`, so the pointer this section used to carry resolved to nothing
for anyone who cloned the repository, and there was no licence text in it at
all. Worth recording as the kind of thing that is invisible from inside a
working tree where the file happens to be sitting there.
