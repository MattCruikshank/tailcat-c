...
/* ---- parse -------------------------------------------------------------
 *
 * The output is JSON because upstream's is, and byte-identical to it because
 * anything less is a difference someone's jq expression finds for us. It is
 * checked that way: `make parse-interop` runs both binaries over addresses
 * generated by the real Go library and requires the two outputs to match
 * exactly.
 *
 * Three consequences of "identical" worth stating, because each reverses
 * something this file used to do:
 *
 *   - The fields are upstream's *wire* names and order, not ours. Upstream's
 *     parse calls ParseAddrRaw, so what is printed is what the CBOR carried,
 *     with nothing derived. tc_addr_parse_raw exists for exactly this.
 *
 *   - Absent means absent. Go's omitempty drops zero numbers and empty
 *     strings, so a region that encoded no ID prints no RegionID, rather
 *     than the 1 our restoring parser would have inferred.
 *
 *   - The pre-shared key is printed. This file used to refuse, on the
 *     grounds that it is a secret -- and it is, but the argument does not
 *     survive contact with the fact that the whole address is one and the
 *     user just typed it on the command line, where it is already in
 *     /proc/<pid>/cmdline for anyone on the machine to read. Printing a
 *     subset of what was passed in reveals nothing that was not already
 *     revealed, and omitting it would make our output silently disagree
 *     with upstream's about what an address contains.
 */

/* print_json_string writes a quoted, escaped JSON string. */
static void print_json_string(const char *s)
{
	char buf[6 * TC_DNS_NAME_MAX + 1];

	if (tc_json_escape(buf, sizeof buf, s) != TC_OK) {
		/* Cannot happen with the fixed-size fields of a parsed address --
		 * six bytes of escape per input byte is the worst case and the
		 * buffer allows for it -- but a JSON writer that silently emits
		 * half a string is worse than one that stops. */
		fprintf(stderr, "tailcat-c: internal: string too long to escape\n");
		exit(1);
	}
	printf("\"%s\"", buf);
}

static void print_json_hex_key(const char *prefix, const uint8_t *p, size_t n)
{
	printf("\"%s", prefix);
...
/* One "Name": value member, with the comma-before-newline bookkeeping that
 * JSON requires and nothing else does. */
static void member(bool *first, int indent, const char *name)
{
	if (!*first)
		printf(",\n");
	*first = false;
...
static void print_node_json(const tc_derp_node *n, int indent)
{
	bool first = true;

	printf("{\n");
	if (n->name[0] != '\0') {
		member(&first, indent + 4, "Name");
		print_json_string(n->name);
	}
	if (n->region_id != 0) {
		member(&first, indent + 4, "RegionID");
		printf("%lld", (long long)n->region_id);
	}
	if (n->hostname[0] != '\0') {
		member(&first, indent + 4, "HostName");
		print_json_string(n->hostname);
	}
	if (n->cert_name[0] != '\0') {
		member(&first, indent + 4, "CertName");
		print_json_string(n->cert_name);
	}
	if (n->ipv4[0] != '\0') {
		member(&first, indent + 4, "IPv4");
		print_json_string(n->ipv4);
	}
	if (n->ipv6[0] != '\0') {
		member(&first, indent + 4, "IPv6");
		print_json_string(n->ipv6);
	}
	if (n->stun_port != 0) {
		member(&first, indent + 4, "STUNPort");
		printf("%d", (int)n->stun_port);
	}
	if (n->derp_port != 0) {
		member(&first, indent + 4, "DERPPort");
		printf("%d", (int)n->derp_port);
	}
	if (n->insecure_for_tests) {
		member(&first, indent + 4, "InsecureForTests");
		printf("true");
	}
	printf("\n%*s}", indent, "");
}

static void print_region_json(const tc_derp_region *r, int indent)
{
	bool first = true;
	...
	if (r->region_id != 0) {
		member(&first, indent + 4, "RegionID");
		printf("%lld", (long long)r->region_id);
	}
	if (r->region_code[0] != '\0') {
		member(&first, indent + 4, "RegionCode");
		print_json_string(r->region_code);
	}
	if (r->region_name[0] != '\0') {
		member(&first, indent + 4, "RegionName");
		print_json_string(r->region_name);
	}
	if (r->num_nodes > 0) {
		member(&first, indent + 4, "Nodes");
		printf("[\n");
		for (size_t i = 0; i < r->num_nodes; i++) {
			printf("%*s", indent + 8, "");
			print_node_json(&r->nodes[i], indent + 8);
			printf("%s\n", i + 1 < r->num_nodes ? "," : "");
		}
		printf("%*s]", indent + 4, "");
	}
...
static int cmd_parse(const char *addr_str)
{
	...
	int rc = tc_addr_parse_raw(&ci, addr_str, strlen(addr_str));
	if (rc != TC_OK) {
		fprintf(stderr, "tailcat-c: %s\n", tc_strerror(rc));
		return 1;
	}
	...
	printf("{\n");
	member(&first, 4, "ServerPublic");
	print_json_hex_key("nodekey:", ci.server_public, TC_NODE_KEY_LEN);
	if (ci.has_disco_public) {
		member(&first, 4, "ServerDiscoPublic");
		print_json_hex_key("discokey:", ci.server_disco_public,
		                   TC_DISCO_KEY_LEN);
	}
	if (ci.has_preshared_key) {
		member(&first, 4, "PresharedKey");
		print_json_hex_key("psk:", ci.preshared_key, TC_PSK_LEN);
	}
	if (ci.num_regions > 0) {
		member(&first, 4, "Region");
		printf("[\n");
		for (size_t i = 0; i < ci.num_regions; i++) {
			printf("%*s", 8, "");
			print_region_json(&ci.regions[i], 8);
			printf("%s\n", i + 1 < ci.num_regions ? "," : "");
		}
		printf("%*s]", 4, "");
	}
	if (ci.region_id != 0) {
		member(&first, 4, "RegionID");
		printf("%lld", (long long)ci.region_id);
	}
	printf("\n}\n");
	return 0;
}
...
int main(int argc, char **argv)
{
	...
	for (int i = 1; i < argc; i++) {
		 ...
		 * -- `--bind=0.0.0.0`, `--key=default`, `--region=nyc`,
		 * `--allow=nodekey:...` -- so a reader copying from it met "unknown
		 * flag" on nearly every example. The two spellings are the same
		 * flag, which is why the split happens here rather than in twenty
		 * branches below. */
		if (a[0] == '-' && a[1] == '-') {
		...
		/* val() is the value for a flag that needs one: what followed the
		 * `=` if there was one, otherwise the next argument. NULL means the
		 * user wrote a flag and then nothing, which used to fall through to
		 * "unknown flag" and blame the wrong thing. */
		const char *val = NULL;
#define NEED_VAL()                                                            \
	do {                                                                      \
		val = (inl != NULL) ? inl : (i + 1 < argc ? argv[++i] : NULL);        \
		if (val == NULL) {                                                    \
			fprintf(stderr, "tailcat-c: %s needs a value\n", a);              \
			return 2;                                                         \
		}                                                                     \
	} while (0)
		/* And the mirror of it: a switch given `=something` was meant to be
		 * a boolean, as in upstream's `--psk=false`. Anything else is a
		 * typo worth reporting rather than quietly reading as "on". */
#define BOOL_VAL(dflt)                                                        \
	(inl == NULL                                                              \
	     ? (dflt)                                                             \
	     : (strcmp(inl, "true") == 0 || strcmp(inl, "1") == 0)                \
	           ? true                                                         \
	           : (strcmp(inl, "false") == 0 || strcmp(inl, "0") == 0)         \
	                 ? false                                                  \
	                 : (fprintf(stderr,                                       \
	                            "tailcat-c: %s takes true or false, not "     \
	                            "\"%s\"\n",                                   \
	                            a, inl),                                      \
	                    exit(2), false))

		if (strcmp(a, "-v") == 0 || strcmp(a, "--verbose") == 0) {
			g_verbose = BOOL_VAL(true);
		} else if (strcmp(a, "--insecure") == 0) {
			insecure = BOOL_VAL(true);
		} else if (strcmp(a, "--relay") == 0) {
			NEED_VAL();
			relay = val;
			...
			full_address = BOOL_VAL(true);
		} else if (strcmp(a, "--allow") == 0) {
			...
			NEED_VAL();
			if (tc_allow_parse(&allow, val) != TC_OK) {
				fprintf(stderr, "tailcat-c: %s\n", tc_allow_error_string());
				return 2;
			}
		} else if (strcmp(a, "--client") == 0) {
			gk_client = BOOL_VAL(true);
		} else if (strcmp(a, "--force") == 0) {
			gk_force = BOOL_VAL(true);
		} else if (strcmp(a, "--delete") == 0) {
			gk_delete = BOOL_VAL(true);
		} else if (strcmp(a, "--list") == 0) {
			gk_list = BOOL_VAL(true);
		} else if (strcmp(a, "--region") == 0) {
			NEED_VAL();
			gk_region = val;
		} else if (strcmp(a, "--psk") == 0) {
			/* Upstream's spelling, and the only one its README uses:
			 * `--psk=false`. Ours was --no-psk, which is kept because it is
			 * what this program's own documentation has always said. */
