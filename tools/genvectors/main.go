...
func main() {
	...
	// ---- Ed25519 ------------------------------------------------------
	//
	// Generated by Go's crypto/ed25519, which is RFC 8032 compliant -- and
	// fed the RFC's own seeds and messages, so the signatures below *are*
	// the RFC's published vectors, arrived at rather than transcribed. One
	// of them is also hard-coded in the test as a fixed point, so a
	// generator pointed at the wrong thing cannot agree with itself.
	b.line("/* Ed25519, via crypto/ed25519 with RFC 8032's own inputs. */")
	b.line("static const struct {")
	b.line("\tconst char *name, *seed, *pub, *msg, *sig;")
	b.line("} kEd25519Vectors[] = {")
	{
		type edCase struct{ name, seed, msg string }
		cases := []edCase{
			// RFC 8032 section 7.1, in order.
			{"rfc8032-1",
			...
			// Lengths that exercise the hash's block boundary, which is
			// where a length-handling bug hides.
			{"len-63", "0102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f20",
				strings.Repeat("5a", 63)},
			{"len-64", "0102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f20",
				strings.Repeat("5a", 64)},
			{"len-65", "0102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f20",
				strings.Repeat("5a", 65)},
			{"len-1000", "0102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f20",
				strings.Repeat("a5", 1000)},
			// A seed of all zeros and one of all ones, because the edges of
			// the clamping are worth pinning.
			{"seed-zero", strings.Repeat("00", 32), "68656c6c6f"},
			{"seed-ff", strings.Repeat("ff", 32), "68656c6c6f"},
		}
		for _, c := range cases {
			...
			if !ed25519.Verify(pub, msg, sig) {
				panic("go refused its own signature")
			}
			b.line("\t{ %q, %q, %q, %q, %q },", c.name, c.seed, h(pub),
				c.msg, h(sig))
		}
	}
	b.line("};")
